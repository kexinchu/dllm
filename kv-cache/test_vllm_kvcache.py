"""
KV-Cache Layout Determinism Test — vLLM
========================================
All tests batch_size=1, orthogonal to batch invariance.
Tests whether prefix caching and chunked prefill affect the first generated token.

Scenario A: Prefix Cache Hit vs Fresh Prefill
  - Engine 1 (no cache):  send full prompt → first token
  - Engine 2 (with cache): send prefix first to warm cache,
                            then send full prompt → prefix hits cache → first token
  - Compare first tokens

Scenario B: Chunked Prefill
  - Engine 1: chunked_prefill=False → full prefill
  - Engine 2: chunked_prefill=True, various max_num_batched_tokens
  - Compare first tokens
"""
...
# ---------------------------------------------------------------------------
# Diverse prompts: 8 prefixes × 30 suffixes = 240 unique combos, cycle to 1000
# ---------------------------------------------------------------------------
PREFIXES = [
...
def cleanup_engine():
    """Force cleanup GPU memory between engine instances."""
    gc.collect()
...
def run_scenario_a_vllm(num_runs=1000):
    """
    Compare first generated token:
    - Engine with prefix caching OFF: fresh prefill every time
    - Engine with prefix caching ON: warm cache with prefix, then generate

    To trigger a real cache hit, we send TWO requests to the cached engine:
      1. A "dummy" prompt that shares the same prefix → populates cache
      2. The real full prompt → prefix should hit cache
    """
    from vllm import LLM, SamplingParams

    print("\n" + "=" * 70)
    print("vLLM Scenario A: Prefix Cache Hit vs Fresh Prefill")
    print("=" * 70)

    pairs = build_prompt_pairs(num_runs)

    # Greedy, 1 token, get logprobs
    sp = SamplingParams(temperature=0.0, max_tokens=1, logprobs=5)

    # --- Phase 1: No prefix caching (baseline) ---
    print("  Phase 1: Generating with prefix caching DISABLED...")
    llm_nocache = LLM(
        model=MODEL_PATH,
        dtype="bfloat16",
    ...
    # Send all full prompts in one batch (each is independent, batch_size=1 semantics
    # because vLLM processes each prompt independently in its paged attention)
    full_prompts = [p["full"] for p in pairs]
    outputs_nocache = llm_nocache.generate(full_prompts, sp)

    # Extract first tokens
    tokens_nocache = {}
    for i, out in enumerate(outputs_nocache):
        tok_id = out.outputs[0].token_ids[0]
        tok_text = out.outputs[0].text
        tokens_nocache[i] = {"token_id": tok_id, "text": tok_text}

    del llm_nocache
    cleanup_engine()

    # --- Phase 2: With prefix caching ---
    print("  Phase 2: Generating with prefix caching ENABLED...")
    llm_cache = LLM(
        model=MODEL_PATH,
        dtype="bfloat16",
    ...
    # Step 2a: Warm the cache — for each unique prefix, send one prompt
    unique_prefixes = list(set(p["prefix"] for p in pairs))
    print(f"    Warming cache with {len(unique_prefixes)} unique prefixes...")
    # Send prefix + a dummy suffix to populate the KV cache for the prefix
    warmup_prompts = [p + "Hello" for p in unique_prefixes]
    sp_warmup = SamplingParams(temperature=0.0, max_tokens=1)
    _ = llm_cache.generate(warmup_prompts, sp_warmup)

    # Step 2b: Now send the real prompts — prefix should hit cache
    print(f"    Generating {num_runs} prompts (prefix cache should be hit)...")
    outputs_cache = llm_cache.generate(full_prompts, sp)

    tokens_cache = {}
    for i, out in enumerate(outputs_cache):
        tok_id = out.outputs[0].token_ids[0]
        tok_text = out.outputs[0].text
        tokens_cache[i] = {"token_id": tok_id, "text": tok_text}
    ...
    for i in range(total):
        if tokens_nocache[i]["token_id"] != tokens_cache[i]["token_id"]:
            flips += 1
    ...
    print(f"\n  RESULT: first_token_flip = {flips}/{total} ({flips/total*100:.2f}%)")
    if flip_examples:
...
def run_scenario_b_vllm(num_runs=1000):
    """
    Compare first generated token with different chunked prefill settings.
    Baseline: chunked_prefill=False
    Variants: chunked_prefill=True with max_num_batched_tokens = 128, 64, 32
    """
    from vllm import LLM, SamplingParams

    print("\n" + "=" * 70)
    print("vLLM Scenario B: Chunked Prefill — Different Chunk Sizes")
    print("=" * 70)
    ...
    print("  Baseline: chunked_prefill=False...")
    llm_base = LLM(
        model=MODEL_PATH,
        dtype="bfloat16",
        enable_prefix_caching=False,
    ...
    outputs_base = llm_base.generate(prompts, sp)
    tokens_base = []
    for out in outputs_base:
        tokens_base.append({
            "token_id": out.outputs[0].token_ids[0],
            "text": out.outputs[0].text,
        })
    del llm_base
    cleanup_engine()

    # --- Variants: chunked prefill with different chunk sizes ---
    chunk_configs = [128, 64, 32]
    results = {}

    for chunk_size in chunk_configs:
        print(f"  Chunked prefill: max_num_batched_tokens={chunk_size}...")
        llm_chunk = LLM(
            model=MODEL_PATH,
            dtype="bfloat16",
            enable_prefix_caching=False,
            ...
            max_num_seqs=min(chunk_size, 32),
            max_model_len=4096,
            gpu_memory_utilization=0.5,
        )
        outputs_chunk = llm_chunk.generate(prompts, sp)
        ...
        for i, out in enumerate(outputs_chunk):
            tok_id = out.outputs[0].token_ids[0]
            tok_text = out.outputs[0].text
            if tok_id != tokens_base[i]["token_id"]:
                flips += 1
                flip_examples.append({
                    "idx": i,
                    "prompt": prompts[i][-60:],
                    "token_base": tokens_base[i]["text"],
                    "token_chunked": tok_text,
                })
...
def main():
    ...
    parser.add_argument("--output", type=str, default="vllm_kvcache_results.json")
    args = parser.parse_args()

    scenarios = args.scenarios.lower().split(",")
    all_results = {"engine": "vllm", "num_runs": args.num_runs}

    t0 = time.time()

    if "a" in scenarios:
        all_results["scenario_a_prefix_cache"] = run_scenario_a_vllm(args.num_runs)

    if "b" in scenarios:
        all_results["scenario_b_chunked_prefill"] = run_scenario_b_vllm(args.num_runs)
