"""
DetermLLM: Cross-Model Validation on Llama-3.1-8B (KV-Cache)
=============================================================
Same production-path experiment as run_kvcache_determ.py but on a larger model.
Uses smaller batch sizes due to 48GB memory limit.

Output: research/exp_kvcache_8b.json
"""
...
OUT_FILE   = os.path.join(DLLM_DIR, 'exp_kvcache_8b.json')
...
GEN_LEN     = 64
BATCH_SIZES = [2, 4, 8, 16, 32]   # 8B model; skip bs=64 to stay in memory
...
print(f"\n{'='*65}")
print("SUMMARY: KV-Cache Determinism — Llama-3.1-8B")
print(f"{'='*65}")
for cond, cres in all_results.items():
    m = '✓' if cres['all_sequences_match'] else '✗'
    print(f"  {cond:<30} {m:>4} mm={cres['total_token_mismatches']:>4} "
