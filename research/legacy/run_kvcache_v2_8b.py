"""
DetermLLM: Production KV-Cache Experiment v2
=============================================
Adds Condition D: F.linear + GEMV/GEMM padding fix (pad_m1=True).

The GEMV/GEMM boundary fix pads M=1 decode inputs to M=2 before the
cuBLASLt call, forcing cuBLAS to use the GEMM code path for all batch
sizes.  Op-level verification shows this closes the ~7.8e-3 boundary gap
for all 1B attention projection shapes (q/k/v/o_proj, K=2048).

Conditions:
  A: BF16 baseline
  B: DetermLLM F.linear (precision amplification, no padding fix)
  D: DetermLLM F.linear + padding fix (pad_m1=True)

Output: research/exp_kvcache_v2_8b.json
"""
...
OUT_FILE   = os.path.join(DLLM_DIR, 'exp_kvcache_v2_8b.json')
...
GEN_LEN     = 64
BATCH_SIZES = [2, 4, 8, 16, 32]  # 8B: skip bs=64 for 48GB memory
...
def duplicate_kv(past_kv, n):
    if HAS_DYNAMIC_CACHE and isinstance(past_kv, DynamicCache):
        ...
        return dup
    return tuple((k.repeat(n,1,1,1), v.repeat(n,1,1,1)) for k, v in past_kv)


def decode_with_kv(model, first_logits, past_kv_1, batch_size, gen_len, device):
    token_ids, token_lps = [], []
...
def run_condition(model, tokenizer, device, label):
    ...
    for p_idx, prompt in enumerate(PROMPTS):
        ...
        ref_toks, ref_lps = decode_with_kv(
            model, first_logits_1, past_kv_1, 1, GEN_LEN, device)
        ref_hash = seq_hash(ref_toks)
        prompt_res = {'ref_hash': ref_hash, 'batches': {}}

        for bs in BATCH_SIZES:
            bs_toks, bs_lps = decode_with_kv(
                model, first_logits_1, past_kv_1, bs, GEN_LEN, device)
...
all_results['D_linear_pad_m1'] = run_condition(model, tokenizer, device, 'D_linear_pad_m1')

determ_llm.disable()

print(f"\n{'='*65}")
print("SUMMARY: Production KV-Cache v2 (bs=1 prefill, bs=N decode)")
print(f"{'='*65}")
print(f"{'Condition':<35} {'All Match?':>10} {'Tot MM':>8} {'Avg|ΔlogP|':>12}")
print('-' * 68)
for cond, cres in all_results.items():
    m = '✓' if cres['all_sequences_match'] else '✗'
    print(f"  {cond:<35} {m:>10} {cres['total_token_mismatches']:>8} "
