...
import torch
# ── Vendored from vLLM 0.10.0 (model_executor/layers/batch_invariant.py).
# Imports stubbed for use outside the vllm runtime.
import logging
import triton
...
class _NoOpEnvs:
    """Minimal stub replacing vllm.envs (only the few names we use)."""
    VLLM_BATCH_INVARIANT = True   # already enabled when this module is imported
envs = _NoOpEnvs()

class _PlatformStub:
    @staticmethod
    def is_device_capability(major):
        cap = __import__("torch").cuda.get_device_capability()
        return cap[0] == major
    @staticmethod
    def is_device_capability_family(major):
        cap = __import__("torch").cuda.get_device_capability()
        # crude: family by tens digit (80 → Ampere, 90 → Hopper, 100 → Blackwell)
        return cap[0] == major // 10
current_platform = _PlatformStub()

def num_compute_units():
    return __import__("torch").cuda.get_device_properties(0).multi_processor_count

def is_torch_equal_or_newer(version_str: str):
    import torch
    cur = tuple(int(x) for x in torch.__version__.split("+")[0].split(".")[:2])
    want = tuple(int(x) for x in version_str.split(".")[:2])
    return cur >= want

# Sentinel value for AttentionBackendEnum-typed args we don't actually use
